 ...
 */
static inline mp_size_t lmmp_digits_(mp_srcptr numa, mp_size_t na, int base) {
    int mslbits = 0;
    if (numa) {
        do {
            if (na == 0)
 ...
 */
static inline mp_size_t lmmp_limbs_(const mp_byte_t* src, mp_size_t len, int base) {
    if (src) {
        do {
            if (len == 0)
                return 1;
