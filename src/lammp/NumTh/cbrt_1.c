...
mp_limb_t lmmp_cbrt_3_(mp_limb_t a0, mp_limb_t a1, mp_limb_t a2) {
    ...
    mp_limb_t a_hi;
    mp_bitcnt_t bits, exact_bits;
    if (a2 == 0) {
        bits = LIMB_BITS + lmmp_limb_bits_(a1);
    } else {
        bits = LIMB_BITS * 2 + lmmp_limb_bits_(a2);
    }

    exact_bits = LIMB_BITS - (1 - (bits % 3) + 3) % 3;
    bits = bits - exact_bits;

    if (a2 == 0) {
        if (bits < LIMB_BITS) {
            a_hi = a0 >> bits;
            a_hi |= a1 << (LIMB_BITS - bits);
        } else if (bits == LIMB_BITS) {
            a_hi = a1;
        } else {
            a_hi = a1 >> (bits - LIMB_BITS);
        }
    } else {
        lmmp_debug_assert(bits > LIMB_BITS);
        mp_bitcnt_t bits2 = bits - LIMB_BITS;
        if (bits2 < LIMB_BITS) {
            a_hi = a1 >> bits2;
            a_hi |= a2 << (LIMB_BITS - bits2);
        } else if (bits2 == LIMB_BITS) {
            a_hi = a2;
        } else {
            a_hi = a2 >> (bits2 - LIMB_BITS);
        }
    }

    mp_limb_t ret_lo, ret_hi;
    ret_lo = lmmp_cbrt_chebyshev_(a_hi);
    ret_hi = ret_lo + 1;
    lmmp_debug_assert(bits % 3 == 0);
    ret_lo <<= bits / 3;
    ret_hi <<= bits / 3;

    mp_limb_t t[3];
    mp_limb_t a[3] = {a0, a1, a2};

    // still working while ret_hi = 0, which means ret_hi is B
    while (ret_lo < ret_hi - 1) {
        mp_limb_t mid = ret_lo + (ret_hi - ret_lo) / 2;
        lmmp_cube_3_(t, mid);
